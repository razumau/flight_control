...
class FlightControl::BasicAuthenticationTest < ActionDispatch::IntegrationTest
  test "unconfigured basic auth is closed" do
    with_http_basic_auth do
      get flight_control.application_queues_url(@application), headers: auth_headers("dev", "secret")
  ...
  test "fail to authenticate with wrong credentials" do
    with_http_basic_auth(user: "dev", password: "secret") do
      get flight_control.application_queues_url(@application), headers: auth_headers("dev", "wrong")
      assert_response :unauthorized
    end
  end
